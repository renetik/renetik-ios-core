...
public protocol CSResponseController {
    func show<T: CSAnyProtocol>(_ response: CSResponse<T>, _ title: String, _ progress: Bool, _ canCancel: Bool,
                            _ failedDialog: Bool, _ onSuccess: ((T) -> Void)?) -> CSResponse<T>
}

public extension CSResponseController {
    @discardableResult
    func show<Data: CSAnyProtocol>(_ response: CSResponse<Data>, title: String = .requestLoading,
