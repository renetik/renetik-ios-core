...
public extension UIScreen {
    ...
    class var isZoomed: Bool {
        main.nativeScale >= main.scale
    }

    class var isRetina: Bool {
        main.scale >= 2.0
    }
    ...
    class var isThin: Bool {
        isPortrait && UIDevice.isPhone
    }

    class var isUltraThin: Bool {
        isPortrait && isSmallPhone
    }

    class var isWide: Bool {
        !isThin
    }

    class var isUltraWide: Bool {
        UIDevice.isTablet && isLandscape
    }

    class var isShort: Bool {
        isLandscape && UIDevice.isPhone
    }

    class var isUltraShort: Bool {
        isLandscape && isSmallPhone
    }

    class var isTall: Bool {
        !isShort
    } // tablet || portrait
    class var isUltraTall: Bool {
        UIDevice.isTablet || !isSmallPhone && isPortrait
    }
